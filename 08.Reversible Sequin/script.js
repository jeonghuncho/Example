(() => {
  ...
  function prepareLuma(data, cols, rows) {
    ...
    for (let index = 0, pixel = 0; index < count; index += 1, pixel += 4) {
      ...
      state.motion[index] = state.previousLuma ? Math.abs(value - state.previousLuma[index]) : 0;
    }
  }
  ...
  function drawWaitingScene(time) {
    drawPanelBase();
    const cell = clamp(state.width / 118, 7.4, 10.8);
  ...
  function drawLiveScene(time) {
    const cell = clamp(state.width / 118, 7.2, 10.8);
    const cols = Math.max(64, Math.floor(state.width / cell));
    const rows = Math.max(42, Math.floor(state.height / cell));
    const cellW = state.width / cols;
    const cellH = state.height / rows;
    ...
    drawFlowers(time);
    drawPanelDetails(time);
    state.previousLuma = new Uint8ClampedArray(state.luma);
  }

  function draw(time) {
    requestAnimationFrame(draw);

    if (time - state.lastRender < 32) return;
